...
class MockPortalScraperImpl(
    ...
    companion object {
        ...
        private const val WIFI_NOT_PRIMARY_MSG =
            "But you may not enjoy it because your internet is on!"
    }

    override fun performLogin(username: String, password: String): Flow<Resource<String>> = flow {
...
//            Timber.d("Page: ${page.asNormalizedText()}")
            ...
            val isWifiPrimary =
                isJmiWifi(forceWifi = false) && isInternetAvailable(isCheckingForWifi = false).getOrNull() ?: true
    ...
    override suspend fun performLogout(): Result<String> = withContext(Dispatchers.IO) {
    ...
    override suspend fun isJmiWifi(forceWifi: Boolean): Boolean = withContext(Dispatchers.IO) {
        try {
    ...
    override suspend fun isInternetAvailable(isCheckingForWifi: Boolean): Result<Boolean> =
